<script lang="ts">
  ...
  export let model: DropDownOption | DropDownOption[]
  export let zIndex = 1000
  export let optionSelectedColor = '#CCC'
  export let optionHighlightedColor = '#777'
  export let onChange: (value: DropDownOption | DropDownOption[] | undefined) => void
  ...
  function isDropDownOptionSelected(option: DropDownOption) {
    ...
    return multiple ? (model as DropDownOption[]).includes(option) : model === option
  }
  ...
  function handleClickOutside (event: MouseEvent) {
    if (containerRef && !(containerRef as HTMLDivElement).contains(event.target as Node)) {
      isListOpen = false
    }
  }
  ...
  function containerSelectedClicked(event: MouseEvent | KeyboardEvent) {
